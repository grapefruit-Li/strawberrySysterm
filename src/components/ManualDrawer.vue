...
const v1Manual: Record<string, { title: string; items: string[] }> = {
  ...
  config: {
    title: '模拟配置',
    items: [
      '【气象配置】选择区域后自动生成180天气象数据，也可上传DSSAT .WTH文件',
      '【区域选择】内置4个典型产区：Florida Balm(亚热带)、California Salinas(地中海)、上海(亚热带季风)、昆明(高原)',
      '【气象数据】包含太阳辐射、最高/最低温、降水量等逐日数据',
      '【土壤配置】可添加/删除土层，编辑每层的容重、砂粒、粘粒、有机质、pH、有效含水量',
      '【品种选择】从13个内置品种中选择，选中后自动加载DSSAT CROPGRO参数',
      '【品种参数】展示选中品种的遗传系数（出苗积温、开花积温、最大LAI等），由品种数据自动填充',
  ...
  data: {
    title: '数据管理',
    items: [
      '【气象数据】查看和编辑当前气象配置，支持上传DSSAT格式.WTH文件',
      '【土壤数据】查看和编辑土壤剖面参数',
      '【品种数据】查看品种参数详情',
...
const v2Manual: Record<string, { title: string; items: string[] }> = {
  ...
  'basic-info': {
    ...
    items: [
      ...
      '【气象文件】可选上传DSSAT格式.WTH文件替代默认气象数据',
