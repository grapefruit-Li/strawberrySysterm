...
/* 处理天气文件上传 */
async function handleWeatherUpload(event: Event) {
      ...
      <div class="field-card full-width">
        <div class="card-header">
          ...
          <div style="display:flex;align-items:center;gap:8px">
            <button class="card-badge" style="cursor:pointer;background:var(--accent-blue);color:#fff;border:none" @click="downloadWeatherTemplate">下载模板</button>
            <span class="card-badge">WTH/Excel</span>
          </div>
        </div>
        <div class="file-upload-card" @click="triggerWeatherUpload">
          <template v-if="!uploadedFileName">
            ...
            <div class="upload-hint">支持 .WTH, .xlsx, .xls 格式</div>
          </template>
          <template v-else>
